...
const SHAKE_DECAY = 0.012; // затухание тряски за секунду (порт shake -= dt*0.012)

export class CameraRig {
  zoom = DEFAULT_ZOOM;
  // Величина тряски камеры; заводится Scene на explosionStarted (Task 8), гасится тут же.
  shake = 0;
  ...
  // затем камера ставится на дистанцию zoom вдоль +Z с тряской (порт animate() ~1076-1084).
  ...
  // Тряска гасится безусловно (не зависит от pointerDown — как в эталоне) и джиттерит
  // позицию камеры вокруг штатной дистанции zoom вдоль +Z.
  private applyCamera(dt: number): void {
    ...
    camera.position.set(
      ...
      (Math.random() - 0.5) * this.shake,
      this.zoom + (Math.random() - 0.5) * this.shake,
    );
    camera.lookAt(0, 0, 0);
  }
}
